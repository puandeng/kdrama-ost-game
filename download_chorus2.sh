...
download_chorus() {
  ...
  yt-dlp "ytsearch1:$query" \
    ...
    --download-sections "*0:00-3:30" --no-playlist --quiet --no-warnings \
  ...
  ffmpeg -y -i "$rawfile" -ar 44100 -ac 2 -b:a 128k "$TEMP_DIR/${filename}_full.mp3" 2>/dev/null
  rm -f "$rawfile"

  echo -n "analyzing... "
  local chorus=$(python3 find_chorus.py "$TEMP_DIR/${filename}_full.mp3")

  ffmpeg -y -i "$TEMP_DIR/${filename}_full.mp3" -ss "$chorus" -t 10 \
    -af "afade=t=out:st=9:d=1" -b:a 128k -ar 44100 -ac 2 \
    "$outpath" 2>/dev/null

  rm -f "$TEMP_DIR/${filename}_full.mp3"
  echo "OK (chorus @ ${chorus}s)"
}
